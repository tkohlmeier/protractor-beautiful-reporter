...
app.controller('ScreenshotReportController',['$scope','$http','TitleService', function ($scope, $http, titleService) {
    ...
    this.updateSelectedModal = function (event, index) {
        if(event.keyCode === 37 && index > 0) {
            // ArrowLeft
            this.showHideModal(index, --index)
        } else if(event.keyCode === 39 && index < this.results.length - 1) {
            // ArrowRight
            this.showHideModal(index, ++index)
        }
    };
...
app.run(
    function($rootScope){
        ...
        document.addEventListener('keydown',function(event){
            var key = KeyTranslate(event);
            switch(key){
                case "ArrowLeft":
                    $('.modal:visible button.btn-previous').click();
                    break;
                case "ArrowRight":
                    $('.modal:visible button.btn-next').click();
                    break;
            }
